...
GLMSAST *glms_struct_image_fptr_save(GLMSEval *eval, GLMSAST *ast, GLMSASTList *args,
  ...
  glms_fptr_expect_args(eval, stack, args, (GLMSType[]){ (GLMSType){ .ast_type = GLMS_AST_TYPE_STRING } }, 1);
...
GLMSAST *glms_struct_image_fptr_shade(GLMSEval *eval, GLMSAST *ast,
  ...
  if (!args || args->length <= 0) return ast;
  if (!ast->ptr) GLMS_WARNING_RETURN(ast, stderr, "Image not initialized (ptr = null).\n");
  glms_fptr_expect_args(eval, stack, args, (GLMSType[]){ (GLMSType){ .ast_type = GLMS_AST_TYPE_FUNC } }, 1);
  ...
  GLMSAST* res_ast = glms_env_new_ast_vec3(eval->env, VEC3(gimg->width, gimg->height, 0), false);
  ...
  #if 0
  for (int i = 0; i < 2; i++) {
    GLMSAST* id = glms_env_new_ast(eval->env, GLMS_AST_TYPE_ID, true);
    id->as.id.heap = strdup(signature[i]);
    glms_ast_push(call_ast, id);
  }
  #endif
  ...
  for (int x = 0; x < gimg->width; x++) {
	...
	gimg_set_pixel_vec4(gimg, x, y, result->as.v4);
      }
...
GLMSAST *glms_struct_image_constructor(GLMSEval *eval, GLMSStack *stack,
  ...
  glms_ast_register_function(eval->env, ast, "getPixel", glms_struct_image_fptr_get_pixel);
  glms_ast_register_function(eval->env, ast, "setPixel", glms_struct_image_fptr_set_pixel);
  glms_ast_register_function(eval->env, ast, "make", glms_struct_image_fptr_make);
  glms_ast_register_function(eval->env, ast, "save", glms_struct_image_fptr_save);
  glms_ast_register_function(eval->env, ast, "shade", glms_struct_image_fptr_shade);
